...
public final class SettingsView {
    ...
    private void renderSttSection(GuiGraphicsExtractor g) {
        int x = secX();
        int fy = secY0();
    ...
    private void renderProxySection(GuiGraphicsExtractor g) {
        int x = secX();
        int fy = secY0();
    ...
    private void renderVoiceList(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX(), w = secW();
        var lib = com.dwinovo.numen.client.voice.VoiceLibrary.instance();
    ...
    private void voiceLabel(GuiGraphicsExtractor g, int row, String text) {
    ...
    private void renderSkinSection(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        if (addingSkin) {
            ...
            // 安卓的文件选择器);拖拽入窗仍被 onFilesDrop 静默接住,不再宣传。
            ...
            txt(g, Component.translatable(ModLanguageData.Keys.SKIN_PICK_FILE),
                    rc[0] + 6, rc[1] + 4, TXT);
            if (skinDropped != null) {
                txt(g, Component.translatable(ModLanguageData.Keys.SKIN_LOADED,
                        skinDroppedW + "x" + skinDroppedH), x, dy + 22, OK);
            } else if (skinEditId != null) {
                txt(g, Component.translatable(ModLanguageData.Keys.SKIN_KEEP_OLD), x, dy + 22, TXT_FAINT);
            }
    ...
    private void renderSkinList(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX(), w = secW();
        var lib = com.dwinovo.numen.client.skin.SkinLibrary.instance();
        ...
        for (int i = settingsScroll; i < list.size(); i++) {
            ...
            if (face != null) {
                ...
                net.minecraft.client.gui.components.PlayerFaceExtractor.extractRenderState(g, face, x, ry + 1, 16, true, false, -1);
            }
            int tx = x + 20;
            txt(g, Component.literal(e.name()), tx, ry + 1, TXT);
    ...
    /** 「选择文件…」按钮矩形 {x, y, w, h}(render 与 click 同一套推导)。 */
    private int[] skinChipRect() {
        int x = fx();
        int cy = fy0() + 2 * SET_SP + 4;
        int w0 = font().width(I18n.get(ModLanguageData.Keys.SKIN_PICK_FILE)) + 14;
        return new int[]{x, cy, w0, 16};
    }
    ...
    private void renderThemeSection(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX();
    ...
    private void renderProviderList(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX(), w = secW();
    ...
    private void renderProviderForm(GuiGraphicsExtractor g) {
        int x = fx();
        int fy = fy0();
    ...
    private void renderSettingsNav(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        String[] labels = {
    ...
    private void renderMcpList(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX(), w = secW();
    ...
    private void renderMcpForm(GuiGraphicsExtractor g) {
        int x = fx();
    ...
    private void hoverRow(GuiGraphicsExtractor g, int mouseX, int mouseY, int x, int w, int ry) {
    ...
    private void renderPersonaList(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        int x = secX(), w = secW();
    ...
    private void renderPersonaForm(GuiGraphicsExtractor g) {
        int x = fx();
        int fy = fy0();
