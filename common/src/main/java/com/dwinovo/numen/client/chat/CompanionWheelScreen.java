...
public class CompanionWheelScreen extends Screen {
    ...
    private int radius() {
        // Java 17:没有 Math.clamp,min/max 夹逼等价
        return Math.min(104, Math.max(56, Math.min(this.width, this.height) / 5));
    }
    ...
    public void renderBackground(GuiGraphics g) {
        // 刻意留空:不要默认那层压暗遮罩(render 里自画一层轻压暗)
    }
