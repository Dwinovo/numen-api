...
/** Adds ephemeral runtime state to a request without persisting it into conversation history. */
final class AgentRequestContext {
    ...
    /**
     * Attach XML state to the final request message. Reusing the existing role avoids creating an
     * artificial user turn after every tool result (which would also trigger an unnecessary hybrid
     * screenshot). The source list and its messages remain untouched.
     */
    static List<ConvoState.Msg> attach(List<ConvoState.Msg> messages, String runtimeXml) {
        List<ConvoState.Msg> sanitized = withoutLegacyCurrentTask(messages);
        if (runtimeXml == null || runtimeXml.isBlank()) return sanitized;
        if (sanitized.isEmpty()) return List.of(new ConvoState.Msg.User(runtimeXml));

        List<ConvoState.Msg> out = new ArrayList<>(sanitized);
        int last = out.size() - 1;
        ...
        switch (tail) {
            ...
            case ConvoState.Msg.Assistant assistant -> {
                // At a legal dispatch point an assistant tail has no pending tool calls. Keep its
                // authored text intact and add one request-only user context turn.
    ...
    /** Remove request-time current_task blocks written into history by older builds. */
    private static List<ConvoState.Msg> withoutLegacyCurrentTask(List<ConvoState.Msg> messages) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            int close = content.indexOf(CURRENT_TASK_CLOSE, open + CURRENT_TASK_OPEN.length());
            if (close < 0) continue;
        ...
        return out == null ? messages : List.copyOf(out);
    }
}
