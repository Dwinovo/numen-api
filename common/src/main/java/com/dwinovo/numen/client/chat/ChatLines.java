...
public final class ChatLines {
    ...
    public static void owner(String companionName, String text, boolean voice, boolean queued) {
        MutableComponent line = tinted(
                "你 → " + companionName + ":" + (voice ? "(语音) " : "") + text, OWN);
        if (queued) {
            line.append(tinted("  (排队中,她忙完就看)", TOOL));
        }
        add(line);
    }
    ...
    public static void companion(String companionName, String text) {
        ...
        add(name(companionName).append(tinted(flat, TEXT)));
    }

    /** 系统性提醒(连接失败/配置问题):琥珀警示行——玩家必须知道发生了什么,
     *  不能让失败沉进日志里变成"已读不回"。 */
    public static void notice(String companionName, String text) {
        add(tinted("⚠ " + companionName + ":", 0xE0A53A)
                .append(tinted(text, OWN)));
    }
    ...
    private static MutableComponent tinted(String text, int rgb) {
        return Component.literal(text).withStyle(s -> s.withColor(rgb));
    }
