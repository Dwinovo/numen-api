...
public final class ItemsView {
    ...
    public static void render(GuiGraphics g, Font font, UUID uuid,
        ...
        if (loop != null) {
            ...
            // Java 17:无 Math.clamp,用 min/max 等价改写。
            int barX = c2 + 26, barW = 46, pct = Math.min(100, Math.max(0, loop.contextPercent()));
    ...
    private static void renderStatRow(GuiGraphics g, int x, int y, float value, float max, int vRow) {
        ...
        for (int i = 0; i < units; i++) {
            ...
            GuiCompat.blitHudIcon(g, ix, y, U_EMPTY, vRow, ICON, ICON);
            float v = value - i * 2f;
            if (v >= 2f)      GuiCompat.blitHudIcon(g, ix, y, U_FULL, vRow, ICON, ICON);
            else if (v >= 1f) GuiCompat.blitHudIcon(g, ix, y, U_HALF, vRow, ICON, ICON);
        }
    }
