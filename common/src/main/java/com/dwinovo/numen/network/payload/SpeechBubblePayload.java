 ...
 * ——主人也走这一条路,不做本地特例)。
 */
public record SpeechBubblePayload(UUID entityUuid, byte kind, String text) implements NumenPayload {
    ...
    public static final ResourceLocation ID = new ResourceLocation(Constants.MOD_ID, "speech_bubble");
    ...
    public static SpeechBubblePayload read(FriendlyByteBuf buf) {
        return new SpeechBubblePayload(buf.readUUID(), buf.readByte(), buf.readUtf(MAX_TEXT));
    }
