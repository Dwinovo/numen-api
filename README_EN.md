...
- **Progressive tool disclosure and automatic tool pruning** — the first pulse carries only names and one-line summaries; after the model selects entries through `discover_tools`, full descriptions and parameter schemas enter subsequent requests, with LRU and idle expiry converging the active surface. Completed call/result transactions are automatically removed from request context in conservative batches after a later assistant response proves they were consumed; recent and pending transactions remain, while the append-only log is untouched.
...
- **Optional multimodal visual observation** — structured observation remains the default. Hybrid adds a low-cost 960×540 orientation frame at each user/event turn and relies on structured tool results during the chain; visual uses a fresh high-detail frame (up to 1280×720) on every pulse. Frames are ephemeral and never persisted in history.
...
- **Optional `/goal` self-supervision** — a Goal is a durable completion contract scoped to one companion conversation. At a safe idle boundary, the same model config runs again under an independent, tool-free supervisor system role that audits actual tool/event evidence and returns only complete, continue, or blocked. It is off by default; automatic continuation is capped at 8, and no-tool progress, Stop, `/goal pause`, `/goal clear`, a blocker, or exhausted budget all stop the loop. It needs no second credential and never dispatches worker and supervisor requests concurrently.
...
- **Vision & learning settings** — Settings → Vision selects `structured / hybrid / visual` and independently toggles automatic Skill learning and Goal self-supervision. Numen keeps no vision-model allowlist and never guesses from model ids: sending images is entirely the player's choice. A failed/minimized capture safely retains structured context; an incompatible visual endpoint still surfaces its own error.
- **Goal commands** — enable it in settings or with `/goal on`; create one with `/goal <verifiable objective>`, inspect it with `/goal`, pause with `/goal pause`, explicitly grant a fresh bounded run with `/goal resume`, remove it with `/goal clear`, or globally disable it with `/goal off`.
