...
- **可选的多模态视觉观察**——默认保持原有结构化观察；只有玩家明确选择“混合”或“纯视觉”后，才会把同伴自身第一人称画面发给当前端点。画面按请求临时附加、压缩到 1280×720 JPEG，且不写进历史。
...
- **视觉与学习设置**——面板的“设置 → 视觉”可切换 `结构化 / 混合 / 纯视觉`，也可单独关闭自动技能学习。Numen 不维护或猜测任何“视觉模型名单”：是否发图完全由玩家选择；混合模式失败时可无图重试，纯视觉模式则会保留玩家要求并如实暴露端点错误。
